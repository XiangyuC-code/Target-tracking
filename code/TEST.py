...
def main():
    ...
    for i in range(steps-1):
        ...
        Filter.filter()
        state = np.array(Filter.x_u[:,j]).reshape(4,)
...
if __name__ == "__main__":
    fr = open('err_DQN.txt','w')
    ft = open("time_DQN.txt",'w')
    ...
    while True:
        pass
'''
fer = open('errhis_DQN.txt','w')
error_his/=20
for i in range(800):
    fer.write(str(error_his[i])+'\n')
